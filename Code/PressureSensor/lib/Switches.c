...
#define TOTAL_GPIO 6
const uint8_t sw_ports[] = {GPIO_PORT_P1, GPIO_PORT_P1, GPIO_PORT_P2,
                             GPIO_PORT_P2, GPIO_PORT_P2, GPIO_PORT_P2};
const uint16_t sw_pins[] = {GPIO_PIN3, GPIO_PIN2, GPIO_PIN3,
...
static inline void _check_and_ack_sw(uint8_t sw_id)
{
    ...
    // GPIO_getInterruptStatus shifts even ports by 8
    if((sw_ports[sw_id] & 1) ^ 1)
    {
        status >>= 8;
    }
